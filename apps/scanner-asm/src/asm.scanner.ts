...
import { SurfaceProbe, type ProbeResult } from './surface.probe';
 ...
 *
 * Subdomain enumeration (CT + NS) is additive and domain-only. Findings hang
 * on the job's apex `domain` asset; `location` names the probed host. The
 * worker does not mint assets.
 */
@Injectable()
export class AsmScanner extends BaseScanner {
  ...
  async execute(ctx: ScanContext): Promise<ScanOutcome> {
    ...
    const apexResult = await this.probe.probe(ctx, ctx.job.target);
    const findings = findingsFromProbe(apexResult, this.name);
    const probed: ProbeResult[] = [apexResult];

    let enumerated: string[] = [];
    if (kind === 'domain') {
      enumerated = await this.probe.enumerate(apexResult.host, ctx);
      for (const name of enumerated) {
        if (!ctx.checkDeadline()) throw new Error('Job deadline exceeded');
        if (name === apexResult.host) continue;
        const sub = await this.probe.probe(ctx, { kind: 'domain', externalKey: name });
        probed.push(sub);
        findings.push(...findingsFromProbe(sub, this.name));
      }
    }

    return {
      findings,
      rawOutput: { apex: apexResult, enumerated, probes: probed },
      stats: {
        openPorts: probed.reduce((n, p) => n + p.openPorts.length, 0),
        addresses: probed.reduce((n, p) => n + p.addresses.length, 0),
...
function findingsFromProbe(result: ProbeResult, scannerName: string): RawFinding[] {
