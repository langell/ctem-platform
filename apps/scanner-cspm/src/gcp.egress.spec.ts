...
describe('gcpComputeUrl / gcpStorageBucketsUrl', () => {
  it('derives Compute and Storage hosts from a project id, never a tenant host', () => {
    ...
    expect(gcpStorageBucketUrl('logs-bucket')).toBe(
      'https://storage.googleapis.com/storage/v1/b/logs-bucket',
    );
    expect(gcpStorageBucketIamUrl('logs-bucket')).toBe(
      'https://storage.googleapis.com/storage/v1/b/logs-bucket/iam',
    );
    expect(gcpFirewallUrl('acme-prod', 'allow-ssh')).toBe(
      'https://compute.googleapis.com/compute/v1/projects/acme-prod/global/firewalls/allow-ssh',
    );
  });

  it('refuses a URL-shaped bucket or firewall name', () => {
    expect(() => gcpStorageBucketUrl('https://evil.example')).toThrow(/bucket/);
    expect(() => gcpFirewallUrl('acme-prod', 'https://evil.example')).toThrow(/firewall/);
  });
