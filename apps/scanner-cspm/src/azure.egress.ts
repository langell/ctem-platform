 ...
 * for tokens, `management.azure.com` for ARM), never tenant-writable. Same
 * hosts as Azure discovery — a config/body/query endpoint must not become
 * the destination for `AZURE_*` client secrets.
 */
...
/**
 * Resource group / ARM resource names are path identifiers, never hosts.
 * Slashes and URL-shaped values are refused so they cannot rewrite the ARM host.
 */
export const AZURE_RG_RE = /^[-\w._()]{1,90}$/;
export const AZURE_RESOURCE_NAME_RE = /^[-\w._()]{1,80}$/;

export function assertAzureResourceGroup(name: string): string {
  const trimmed = name.trim();
  if (/^https?:\/\//i.test(trimmed) || trimmed.includes('/') || !AZURE_RG_RE.test(trimmed)) {
    throw new AzureEgressError(
      `Refusing Azure resourceGroup '${name}' — not a valid Azure resource group identifier`,
    );
  }
  return trimmed;
}

export function assertAzureResourceName(name: string, field: string): string {
  const trimmed = name.trim();
  if (/^https?:\/\//i.test(trimmed) || trimmed.includes('/') || !AZURE_RESOURCE_NAME_RE.test(trimmed)) {
    throw new AzureEgressError(
      `Refusing Azure ${field} '${name}' — not a valid Azure resource identifier`,
    );
  }
  return trimmed;
}

/** ARM GET of one resource. nextLink is still allowlisted via allowlistedAzureArmUrl. */
export function azureArmResourceUrl(
  subscriptionId: string,
  ...
  const resource = assertAzureResourceName(resourceName, 'resource');
  if (!providerPath.startsWith('/providers/')) {
    throw new AzureEgressError('Refusing Azure ARM path that is not a provider resource');
  }
  return allowlistedAzureArmUrl(
    `https://${AZURE_ARM_HOST}/subscriptions/${encodeURIComponent(id)}/resourceGroups/${encodeURIComponent(rg)}${providerPath}/${encodeURIComponent(resource)}?api-version=${encodeURIComponent(apiVersion)}`,
  );
}
