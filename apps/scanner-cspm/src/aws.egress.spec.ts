...
describe('awsS3BucketUrl', () => {
  it('uses path-style on s3.amazonaws.com / s3.{region}.amazonaws.com', () => {
    expect(awsS3BucketUrl('us-east-1', 'logs-bucket', { acl: '' })).toBe(
      'https://s3.amazonaws.com/logs-bucket?acl',
    );
    expect(awsS3BucketUrl('eu-west-1', 'logs-bucket', { policyStatus: '' })).toBe(
      'https://s3.eu-west-1.amazonaws.com/logs-bucket?policyStatus',
    );
  });

  it('refuses a URL-shaped or host-like bucket name', () => {
    expect(() => awsS3BucketUrl('us-east-1', 'https://evil.example', { acl: '' })).toThrow(/bucket/);
    expect(() => awsS3BucketUrl('us-east-1', 'evil.example/aws', { acl: '' })).toThrow(/bucket/);
  });
});
