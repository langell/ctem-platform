 ...
 *                whose name is allowlisted (`GITHUB_*`). This is not a
 *                tenant-writable secret store: a tenant-supplied credentialRef
 *                cannot read DATABASE_URL, PATH, INTERNAL_TOKEN_SECRET, or
 *                other replica secrets. Every integration that points at the
 *                same env name shares that replica-global value until a real
 *                per-tenant store (vault:, aws-sm:) exists.
 *
