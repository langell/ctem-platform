...
This repository is a **scaffold**: the architecture, contracts, data model, event flows and service boundaries are in place and compile. GitHub and GitLab.com repository discovery is live, as is AWS cloud-resource inventory (EC2, S3, security groups, Elastic IPs). SCA can ingest an SBOM or resolve lockfiles from a cloned repo. Remaining connectors and scanner internals are stubbed with explicit `TODO`s marking the extension points.
...
Gateway on `http://localhost:3000` (OpenAPI at `/docs`, web UI at `/`). The UI is a thin Nx client of the gateway — org comes from the JWT, never from a header or query. Paste a bearer JWT on `/login` (machine callers present a PAT as `Authorization`, not in the UI). Compose Keycloak imports the `ctem` realm (`OIDC_ISSUER=http://localhost:8080/realms/ctem`) and a demo analyst whose IdP subject is `demo|analyst`, matching the seeded user. Optional Vite HMR: `pnpm dev:web` on port 4200 (proxies `/v1` to the gateway). Tenants list, create, and reorder notify or ticket rules at `/policies`.
...
Scanner internals beyond SCA SBOM ingest and lockfile resolution (image layer walking, IaC parsing, port scanning), remaining discovery connectors (Azure/GCP), distributed scheduling (the schedulers use naive intervals and need leader election before running multiple replicas), and Redis-backed rate limiting. SCA clones only allowlisted `github.com` / `gitlab.com` sources (`cloneUrl` or `github:` / `gitlab:` externalKey); a failed checkout or unusable private-repo credential fails the job. Self-hosted GitLab is later explicit connector config, not an arbitrary hostname. AWS discovery talks only to allowlisted `*.amazonaws.com` via platform `env:AWS_*` — tenant config cannot set an endpoint, and a truncated listing does not archive inventory. `pom.xml`, `*.csproj` and `requirements.txt` are pinned-manifest fallbacks, not dependency graphs.
