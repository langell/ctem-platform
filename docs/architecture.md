...
3. **Cloud connectors.** GitHub and GitLab repository discovery is live (gitlab.com by default; self-hosted via explicit connector `baseUrl`). AWS inventory (EC2, S3, security groups, Elastic IPs → `cloud_resource`) and GCP inventory (GCE, GCS, firewalls, external IPs → `cloud_resource`) are live via the same AssetConnector + scheduler. Azure is later.
...
- Scanner internals beyond SCA SBOM ingest and lockfile resolution are stubbed: image layer walking, IaC parsing, port scanning. Remaining discovery connectors (Azure) are not built yet. AWS and GCP are inventory only — not CSPM scanners.
