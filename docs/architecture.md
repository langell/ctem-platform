...
- Scanner internals beyond SCA SBOM ingest and lockfile resolution are stubbed: image layer walking, IaC parsing, port scanning. Remaining discovery connectors (Kubernetes, ECR, DNS) are not built yet. AWS, GCP, and Azure are inventory only — not CSPM scanners. GHCR lists Packages REST on `api.github.com` only; it does not pull layers.
