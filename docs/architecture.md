...
- Circuit breaker + retry budget (`@ctem/resilience`) is wired on api-gateway `ServiceProxy` (gateway → identity / asset / orchestrator / findings / risk / reporting / notification) and on orchestrator third-party publisher egress (GitHub Checks, GitHub Deployments, GitLab Commit Statuses, GitLab Deployments) via the same `InternalHttpPolicy`. Open circuits fail fast: the gateway maps them to HTTP 503 (no empty/success payload); publisher egress logs and skips the publish (soft-fail preserved — the scan and GET conclusions stay unchanged). Retries are capped (`CTEM_CB_MAX_ATTEMPTS`, default 3) with exponential backoff + jitter, on timeouts and 502–504 only — not 4xx. Publisher circuits are per egress family (`egress:github-api`, `egress:gitlab-api`), not per scan or org. Notification-service Slack incoming webhooks and Jira Cloud issue create use the same `InternalHttpPolicy` on `egress:slack-webhook` and `egress:jira-api` with one attempt (no in-policy retry); an open circuit or a failed send throws so JetStream `notification-dispatch` naks and redelivers (`maxDeliver: 6`). Knobs are platform `CTEM_CB_*` env allowlist (fail closed on invalid); not tenant-writable. Asset-service inventory connectors (GitHub, GitLab, AWS, GCP, Azure, GHCR, ECR, GCR / Artifact Registry, ACR, Docker Hub, Quay, Kubernetes control-plane, and DNS Certificate Transparency) wrap discovery HTTP in that same `InternalHttpPolicy` and fail the sync closed on an open circuit or exhausted retry (no partial archive, no silent success). Inventory circuits are per platform egress family (`egress:github-api` for GitHub and GHCR, `egress:gitlab-api`, `egress:aws-api` for AWS inventory and ECR, `egress:gcp-api` for GCP, Artifact Registry, and the GCP token exchange, `egress:azure-api` for ARM, ACR, and the Azure token exchange, `egress:dockerhub-api`, `egress:quay-api`, `egress:dns-ct`, `egress:k8s-controlplane`), not per org or connector instance. Allowlist refusal stays before the policy and does not trip the circuit. Gateway-auth identity verify, orchestrator → scanners (NATS), and scanner pulls are unchanged.
