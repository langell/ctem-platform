...
6. **Container layer scanning**, then remaining IaC depth, then ASM probing depth. GHCR digest pull on `ghcr.io` is live; Kubernetes workloads are not. ASM subdomain enumeration is live for apex `domain` assets (crt.sh JSON + OS `resolveNs`, findings on the apex asset).
