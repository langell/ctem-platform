...
- Circuit breaker + retry budget (`@ctem/resilience`) is wired on api-gateway `ServiceProxy` (gateway → identity / asset / orchestrator / findings / risk / reporting / notification) and on orchestrator third-party publisher egress (GitHub Checks, GitHub Deployments, GitLab Commit Statuses, GitLab Deployments) via the same `InternalHttpPolicy`. Open circuits fail fast: the gateway maps them to HTTP 503 (no empty/success payload); publisher egress logs and skips the publish (soft-fail preserved — the scan and GET conclusions stay unchanged). Retries are capped (`CTEM_CB_MAX_ATTEMPTS`, default 3) with exponential backoff + jitter, on timeouts and 502–504 only — not 4xx. Publisher circuits are per egress family (`egress:github-api`, `egress:gitlab-api`), not per scan or org. Knobs are platform `CTEM_CB_*` env allowlist (fail closed on invalid); not tenant-writable. Gateway-auth identity verify, orchestrator → scanners (NATS), and other third-party egress (scanner pulls, inventory connectors) are unchanged.
