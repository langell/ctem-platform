...
- Scanner internals beyond SCA SBOM ingest and lockfile resolution: IaC misconfig scanning is live; container image scanning pulls allowlisted `ghcr.io` digests in-process (fail-closed on incomplete layer inventory). Remaining discovery connectors (Kubernetes, ECR, DNS) are not built yet. AWS, GCP, and Azure are inventory only — not CSPM scanners. GHCR discovery lists Packages REST on `api.github.com` only; layer pull is the container scanner on `ghcr.io`.
