...
6. **Container layer scanning**, then remaining IaC depth, then ASM probing depth. GHCR / ECR / GCR / ACR / Docker Hub digest pull is live. Kubernetes discovery inventories managed clusters only (not Deployments/Pods, not kube-apiserver). DNS inventory mints apex + under-apex `domain` assets (`provider=dns_enum`). ASM subdomain enumeration stays findings-only on existing apex `domain` assets (crt.sh JSON + OS `resolveNs`, no mint).
...
- Circuit breaker + retry budget (`@ctem/resilience`) is wired on api-gateway `ServiceProxy` only (gateway → identity / asset / orchestrator / findings / risk / reporting / notification). Open circuits fail fast as HTTP 503 (no empty/success payload). Retries are capped (`CTEM_CB_MAX_ATTEMPTS`, default 3) with exponential backoff + jitter, on timeouts and 502–504 only — not 4xx. Knobs are platform `CTEM_CB_*` env allowlist (fail closed on invalid); not tenant-writable. Gateway-auth identity verify, orchestrator → scanners (NATS), and third-party egress are unchanged.
